...
class bmes:
    ...
    # datadir() will return a default datadir,if you are not happy with that, set this variable.
    CUSTOMDATADIR = None
    # dbfile() will return a default file, if you are not happy with that, set this variable.
    CUSTOMDBFILE = None
    CUSTOMTEMPDIR = None
    CUSTOMPATH = None
...
def tempdir():
    if bmes.CUSTOMTEMPDIR:
        return bmes.CUSTOMTEMPDIR
    import tempfile
...
def datadir():
    if bmes.CUSTOMDATADIR:
        return bmes.CUSTOMDATADIR
    if not hasattr(datadir, 'ret'):
        datadir.ret = ''
    ret = datadir.ret
    if not ret:
        if ispc():
            if os.path.isdir('c:/data/temp'):
                ret = 'c:/data/temp/bmes'
            else:
                ret = 'C:/bmes'
            mkdirif(ret)
        else:
            ret = tempdir()  # TODO: change this to a non-temp folder
        if bmes.PROJECTNAME:
            subdir = bmes.PROJECTNAME
        else:
            subdir = os.path.basename(os.path.dirname(
                selfdir()))+'_'+os.path.basename(selfdir())
        ret = ret+'/'+subdir
        mkdirif(ret)
        datadir.ret = ret
    return ret
...
def isdbtable(cur, tablename):
    if type(cur).__name__ == 'Connection':
        cur = cur.cursor()
...
def condainstall(importname, packagename=None, reinstall=False, condaargs=[]):
    ...
    if reinstall or not isinstalled:
        try:
            ...
            print('FAILED to install ' + packagename)
            print(
                'Try installing the package with Anaconda Prompt. Try running the following:')
...
def bwaexe():
    ...
    if not isfileandnotempty(out):
        if ispc:
            file = downloadurl(
                'http://sacan.biomed.drexel.edu/ftp/prg/bwa.cyg.w64.zip', DATADIR+'/bwa.cyg.w64.zip')
