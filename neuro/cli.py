...
class cli:
    ...
    def plot_connectome(
        ...
        threshold: float = Option(
            0.0, help='Threshold for the magnitude of the cell values'),
        bgcolor: str = Option(
            'black', help='Color for cells below the threshold'),
        cmap: str = Option('jet', help='Matplotlib colormap to use'),
        ext: arg_ext = Option(arg_ext.png, case_sensitive=False,
                              help='File extension for figure'),
        show: bool = Option(False),
        sign: arg_sign = Option(arg_sign.full, case_sensitive=False,
                                help="Plot the 'full' matrix or just the 'positive' or 'negative' side"),
        ...
        symmetric: bool = Option(
            False, help='If the data contains values above and below zero, set the middle point of the range to zero while displaying'),
    ...
    def qc(
        indir: str = Option(...,
                            help='Path to the folder containing the connectomes'),
        outdir: str = Option(..., help="File to write the statistics to"),
        subjects: str = Option("", help='File with subject IDs'),
        file_stub: str = Option(
            ".txt", help='trailing text that follows subject IDs in the file name.'),
        name_pattern: str = Option(
            "R\d*_V\d*", help="Regular expression for extracting subject names from file names."),
        nodes: int = Option(
            ..., help="Number of ROIs in the atlas. Use this flag only if the region IDs are sequential."),
        hemisphere_map: str = Option(
            "", help="File that contains the hemisphere information for the nodes"),
        std: int = Option(
            2.33, help='Label subjects as problematic for being this many standard deviations away from the mean of healthy controls'),
        droplastnode: bool = Option(
            False, help='drop last node of the connectome (brain stem) from strength calculations across hemispheres'),
        modality: arg_modality = Option(
            arg_modality.structural, help="Structural or functional connectome?"),
        zero_diag: bool = Option(False, help='Set the diagonal to zero'),
        title: str = Option("Distribution of Metrics",
        ...
        if subjects != "":
            subjects = [
                x + file_stub for x in open(subjects, 'r').read().splitlines()]
        else:
        ...
        statistics['Total Connectivity'] = \
            metrics.compute_global_metric(
                connectomes, metrics.connectivity_strength)
        statistics['Intrahemispheric Connectivity'] = \
            metrics.compute_global_metric(
                connectomes, metrics.intrahemispheric_strength, hemisphere_map)
        statistics['Interhemispheric Connectivity'] = \
            metrics.compute_global_metric(
                connectomes, metrics.interhemispheric_strength, hemisphere_map)
        statistics['Density'] = \
        ...
        if modality == 'functional':
            ...
            statistics['Total Connectivity'] = \
                metrics.compute_global_metric(
                    neg_connectomes, metrics.connectivity_strength)
            statistics['Intrahemispheric Connectivity'] = \
                metrics.compute_global_metric(
                    neg_connectomes, metrics.intrahemispheric_strength, hemisphere_map)
            statistics['Interhemispheric Connectivity'] = \
                metrics.compute_global_metric(
                    neg_connectomes, metrics.interhemispheric_strength, hemisphere_map)
            statistics['Density'] = \
                metrics.compute_global_metric(neg_connectomes, metrics.density)

        # Calculate z-scores, if indicated by the user
        zscores = {k: stats.zscore(v, subject_order)
        ...
        for stat in statistics:
            outliers_small_name.append([name for name, truth in zip(
                subject_names, zscores[stat] <= -std) if truth])
            outliers_small_score.append([score for score, truth in zip(
                statistics[stat], zscores[stat] <= -std) if truth])
            outliers_small_std.append(
                [score for score in zscores[stat] if score <= -std])

        outliers_large_name = []
        outliers_large_std = []
        outliers_large_score = []
        for stat in statistics:
            outliers_large_name.append([name for name, truth in zip(
                subject_names, zscores[stat] >= std) if truth])
            outliers_large_score.append([score for score, truth in zip(
                statistics[stat], zscores[stat] >= std) if truth])
            outliers_large_std.append(
                [score for score in zscores[stat] if score >= std])
        ...
        with open(outdir + 'Outlier-Report.txt', 'w') as f:
            ...
            # Report on outliers
            f.write(
                '============= Discarded Subjects Due to Missing Node =============\n')
            for subject, node_ct in zip(discard_subjects, discard_subject_node_count):
                f.write('%s: %d nodes\n' % (subject, node_ct))
            f.write('\n')

            f.write(
                '========== Outlier Statistics > %.1f Standard Deviations ==========\n' % std)
            for i, stat in enumerate(statistics):
                f.write('%s ==> ' % stat)
                for j, outlier in enumerate(outliers_large_name[i]):
                    f.write('(%s: %.2f +/- %.2f)\t' % (outlier,
                            outliers_large_score[i][j], outliers_large_std[i][j]))
                f.write('\n\n')
            ...
            for i, stat in enumerate(statistics):
                ...
                for j, outlier in enumerate(outliers_small_name[i]):
                    print(outlier)
                    f.write('(%s: %.2f +/- %.2f)\t' % (outlier,
                            outliers_small_score[i][j], outliers_small_std[i][j]))
                f.write('\n\n')
            ...
            # Write table to file
            f.write(
                '=========== Table with Outlier Status for each Subject ===========\n')
